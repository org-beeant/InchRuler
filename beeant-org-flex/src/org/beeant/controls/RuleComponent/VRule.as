package org.beeant.controls.RuleComponent
{
	...
	import org.beeant.controls.GraphicElement.VRulerGraphicElement;
	
	import spark.components.Group;
	import spark.components.Label;
	import spark.components.supportClasses.SkinnableComponent;

	public class VRule extends SkinnableComponent
	{
		...
		override protected function commitProperties():void 
		{
			...
			if (_bLabelsChanged) {
				...
				rulerGroup.height = _labels.length * labelDistance;
		...
		protected function updatePositionOfLabels ():void 
		{
			if (tickLineLabelInstances && tickLineLabelInstances.length) 
			{
				...
				for (var i:int = 0; i < nLables; i++)
				{
					...
					label.move(LABEL_PADDING, i * labelDistance + LABEL_PADDING);
				}
			}
		}
